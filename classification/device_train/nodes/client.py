...
from data.dataset import simple_collate, cls_batch_preprocess
...
class ClientDist:
    ...
    def __init__(self, model, tokenizer, train_set, valid_set, rank, client_num, init_beta_path, args) -> None:
        ...
        if init_beta_path is None:
            self.prompt = SoftSingleEmbedding(model.backbone.get_input_embeddings(), n_tokens=self.args.n_tokens)
        else:
            self.prompt = SoftSingleEmbedding(model.backbone.get_input_embeddings(), n_tokens=self.args.n_tokens, init_path=init_beta_path)
        logger.add(os.path.join(args.output_dir, 'log.log'))
    
    def save_prompt(self):
        prompt_dic = {'avg':self.prompt.avg.data}
    ...
    def evaluate_loss(self, model, test_loader):
        ...
        for _, batch in enumerate(test_loader):
            ...
            input_ids = cls_batch_preprocess(prompts, self.tokenizer)
            ...
            # calculate how many prediction_token_ids == real next token_ids
    ...
    def train_eval_loss(self):
        ...
        model.backbone.set_input_embeddings(self.prompt)
        ...
        for epoch in range(self.args.client_epochs):
            ...
            test_loader = DataLoader(dataset=self.valid_set, batch_size=self.args.batch_size_eval, num_workers=self.args.num_workers, shuffle=False, collate_fn=simple_collate)
            tmp_loss = self.evaluate_loss(model, test_loader)
            logger.info(f'Epoch {epoch}: Client {self.client_num}: valid loss: {tmp_loss}')
            self.prompt = model.backbone.get_input_embeddings()
...
class ClientDist_beta:
    ...
    def __init__(self, model, tokenizer, train_set, valid_set, rank, client_num, init_beta_path, args) -> None:
        ...
        self.args = args
        if init_beta_path is None:
            self.prompt = SoftSingleEmbedding_beta(model.backbone.get_input_embeddings(), n_tokens=self.args.n_tokens)
        else:
            self.prompt = SoftSingleEmbedding_beta(model.backbone.get_input_embeddings(), n_tokens=self.args.n_tokens, init_path=init_beta_path)
        logger.add(os.path.join(args.output_dir, 'log.log'))
...
class Client_self_train:
    '''
        On-device training baseline client 
    '''
    def __init__(self, model, tokenizer, train_set, valid_set, rank, client_num, args=None) -> None:
        self.model = model
        self.tokenizer = tokenizer
        self.rank = rank
        if rank >= 0:
    ...
    def evaluate_acc(self, model, test_loader):
        model.to(self.device)
        model.eval()
        correct, ttl = 0.0, 0.0
        loss_func = torch.nn.CrossEntropyLoss()
    ...
    def train_test_acc(self):
        loss_func = torch.nn.CrossEntropyLoss()
        train_loader = DataLoader(dataset=self.train_set, batch_size=self.args.batch_size, num_workers=self.args.num_workers, shuffle=True, collate_fn=simple_collate)
        model = deepcopy(self.model)
        model.to(self.device)
        test_loader = DataLoader(dataset=self.valid_set, batch_size=self.args.batch_size_eval, num_workers=self.args.num_workers, shuffle=True, collate_fn=simple_collate)
        acc = self.evaluate_acc(model, test_loader)
        logger.info(f'Init Model, Client: {self.client_num}, Testset Size: {len(self.valid_set)}, valid acc: {acc}')
        ...
        for epoch in range(self.args.client_epochs):
            ...
            logger.info(f'Client {self.client_num}, Train Loss: {train_epoch_loss}, Train PPL: {train_ppl}')
            acc = self.evaluate_acc(model, test_loader)
            logger.info(f'Epoch: {epoch}, Client: {self.client_num}, Testset Size: {len(self.valid_set)}, valid acc: {acc}')
        torch.cuda.empty_cache()
        model.cpu()
        return train_epoch_loss.cpu().item()