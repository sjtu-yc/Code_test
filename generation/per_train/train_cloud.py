...
from dataset_client import batch_preprocess, RedditDataset, create_sampler, create_loader
import utils
from soft_embedding_ import SoftClientEmbedding
...
def train(model, epoch, train_loader, optimizer, lr_scheduler, args, device):
    ...
    for step, batch in tqdm(enumerate(train_loader), total=len(train_loader)):
        prompts, targets, _, client_ids = batch
        ...
        labels = torch.cat([torch.full((input_ids.shape[0], args.n_tokens), -100), labels], dim=1)
        input_ids = input_ids.to(device)
        labels = labels.to(device)
        # put client_ids in the first column of input_ids
        input_ids[:, 0] = client_ids.squeeze()
        loss = model(input_ids=input_ids, labels=labels).loss
        model.module.transformer.wte.sample_flag = True
...
def evaluate(model, test_loader, tokenizer, device, args):
    ...
    for step, batch in tqdm(enumerate(test_loader), total=len(test_loader)):
        prompts, targets, inputs, client_ids = batch
        input_ids = tokenizer(prompts, return_tensors='pt', padding='longest', max_length=50, truncation=True).input_ids
        input_ids = torch.cat([torch.full((input_ids.shape[0], args.n_tokens), 1), input_ids], dim=1).to(device)
        client_ids = torch.tensor(client_ids).to(device)
        input_ids[:, 0] = client_ids.squeeze()
        with torch.no_grad():
            outputs = model.generate(input_ids=input_ids, max_new_tokens=args.max_new_tokens, pad_token_id=tokenizer.pad_token_id, eos_token_id=tokenizer.eos_token_id,
                num_beams=args.num_beams, return_dict_in_generate=True, output_scores=True, num_return_sequences=args.num_beams, use_cache=False)
            model.transformer.wte.sample_flag = True
    ...
    result = [{'label': label, 'pred': pred.strip().replace(' ',''), 'input': input_pinyin, 'context': context,  'top_n': topn, 'scores': scores}
              for label, pred, topn, scores, input_pinyin, context in zip(eval_labels, eval_preds, eval_topn, eval_scores, eval_inputs, eval_contexts)]
    return result
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name_or_path", type=str, default='../model/gpt2-zh-21k_simple_prompt')
    ...
    init_path = f''
    client_embedding = SoftClientEmbedding(num_clients=args.n_clients, wte=model.get_input_embeddings(), n_tokens=args.n_tokens, init_path=init_path) 
    logger.info(f'init path: {init_path}')
    model.set_input_embeddings(client_embedding)
    ...
    train_dataset  = RedditDataset(os.path.join('../data/1407_user/cloud_data', 'Reddit_client_train_data_new.json'), args.split_pinyin, split='train',
        simple_prompt=args.simple_prompt, default_prefix=args.default_prefix)
    test_dataset = RedditDataset(os.path.join('../data/1407_user/cloud_data', 'Reddit_client_test_data_new.json'), args.split_pinyin, split='test',
        simple_prompt=args.simple_prompt, default_prefix=args.default_prefix)
