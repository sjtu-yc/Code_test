...
def acc_from_log_unknown(path, num_epochs=25):
    ...
    num_user = len(init_accs)
    total_data = sum([test_sizes[i] for i in test_sizes.keys()])
    init_acc = sum([init_accs[i]*test_sizes[i] for i in init_accs.keys()]) / total_data
    print(f'Init: {init_acc}')
    # epoch weighted averaged acc
    for i in range(num_epochs):
...
def acc_from_log_noinit(path, num_epochs=25):
    '''
        No initial acc
    '''
    epochs_accs = [{} for _ in range(num_epochs)]
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            if 'Epoch' in line:
                ...
                test_sizes[client_num] = int(line.split(',')[2].split(' ')[-1])
    # init weighted averaged acc
    num_user = len(test_sizes)
    total_data = sum([test_sizes[i] for i in test_sizes.keys()])
    # epoch weighted averaged acc
    for i in range(num_epochs):
        averaged_epoch_acc = sum([epochs_accs[i][j]*test_sizes[j] for j in test_sizes.keys() if j in epochs_accs[i].keys()]) / total_data
        print(f'Epoch {i}: {averaged_epoch_acc}')
    return epochs_accs, test_sizes